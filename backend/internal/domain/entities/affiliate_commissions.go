...
type AffiliateCommissions struct {
	...
	// Commission details
	CommissionAmount  int64 `json:"commission_amount" gorm:"column:commission_amount;type:bigint;not null" validate:"required,gt=0"`
	CommissionRate    float64 `json:"commission_rate" gorm:"column:commission_rate;type:decimal(5,2);not null" validate:"required"`
	TransactionAmount int64 `json:"transaction_amount" gorm:"column:transaction_amount;type:bigint;not null" validate:"required,gt=0"`

	// Commission status
	Status string `json:"status" gorm:"column:status;default:PENDING;check:status IN ('PENDING','APPROVED','PAID','CANCELLED')"`
	...
	EarnedAt  time.Time      `json:"earned_at" gorm:"column:earned_at;autoCreateTime"`
