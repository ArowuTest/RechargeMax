...
type PrizeCategory struct {
	...
	PrizeTemplateID     uint `json:"prize_template_id" gorm:"column:prize_template_id;index;not null"`
	DrawID         *uuid.UUID `json:"draw_id" gorm:"column:draw_id;index"` // Optional: links to specific draw when used
	...
	WinnerCount   int        `json:"winner_count" gorm:"column:winner_count;not null;default:1" validate:"required,gt=0"`
	RunnerUpCount int        `json:"runner_up_count" gorm:"column:runner_up_count;not null;default:1" validate:"required,gte=0"`
