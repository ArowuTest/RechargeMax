...
CREATE TABLE public.data_plans (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    network_id UUID REFERENCES public.network_configs(id) ON DELETE CASCADE,
...
CREATE TABLE public.transactions (
    ...
    data_plan_id UUID REFERENCES public.data_plans(id),
...
CREATE TABLE public.draw_entries (
    ...
    source_subscription_id UUID REFERENCES public.daily_subscriptions(id),
...
CREATE TABLE public.affiliate_clicks (
    ...
    conversion_transaction_id UUID REFERENCES public.transactions(id),
...
CREATE INDEX idx_users_loyalty_tier ON public.users(loyalty_tier);
...
CREATE INDEX idx_network_configs_network_code ON public.network_configs(network_code);
CREATE INDEX idx_network_configs_is_active ON public.network_configs(is_active);
CREATE INDEX idx_network_configs_sort_order ON public.network_configs(sort_order);

-- Data plans indexes
CREATE INDEX idx_data_plans_network_id ON public.data_plans(network_id);
CREATE INDEX idx_data_plans_is_active ON public.data_plans(is_active);
CREATE INDEX idx_data_plans_price ON public.data_plans(price);

-- Transactions indexes
CREATE INDEX idx_transactions_user_id ON public.transactions(user_id);
CREATE INDEX idx_transactions_msisdn ON public.transactions(msisdn);
CREATE INDEX idx_transactions_status ON public.transactions(status);
CREATE INDEX idx_transactions_created_at ON public.transactions(created_at DESC);
CREATE INDEX idx_transactions_network_provider ON public.transactions(network_provider);
CREATE INDEX idx_transactions_payment_reference ON public.transactions(payment_reference);

-- Wheel prizes indexes
CREATE INDEX idx_wheel_prizes_is_active ON public.wheel_prizes(is_active);
CREATE INDEX idx_wheel_prizes_prize_type ON public.wheel_prizes(prize_type);
CREATE INDEX idx_wheel_prizes_sort_order ON public.wheel_prizes(sort_order);

-- Spin results indexes
CREATE INDEX idx_spin_results_user_id ON public.spin_results(user_id);
CREATE INDEX idx_spin_results_transaction_id ON public.spin_results(transaction_id);
CREATE INDEX idx_spin_results_claim_status ON public.spin_results(claim_status);
CREATE INDEX idx_spin_results_created_at ON public.spin_results(created_at DESC);

-- Daily subscriptions indexes
CREATE INDEX idx_daily_subscriptions_user_id ON public.daily_subscriptions(user_id);
CREATE INDEX idx_daily_subscriptions_msisdn ON public.daily_subscriptions(msisdn);
CREATE INDEX idx_daily_subscriptions_subscription_date ON public.daily_subscriptions(subscription_date);
CREATE INDEX idx_daily_subscriptions_status ON public.daily_subscriptions(status);

-- Draws indexes
CREATE INDEX idx_draws_status ON public.draws(status);
CREATE INDEX idx_draws_type ON public.draws(type);
CREATE INDEX idx_draws_start_time ON public.draws(start_time);
CREATE INDEX idx_draws_end_time ON public.draws(end_time);
...
CREATE INDEX idx_draw_entries_source_type ON public.draw_entries(source_type);
...
CREATE INDEX idx_draw_winners_claim_status ON public.draw_winners(claim_status);
...
CREATE INDEX idx_application_metrics_metric_name ON public.application_metrics(metric_name);
CREATE INDEX idx_application_metrics_recorded_at ON public.application_metrics(recorded_at DESC);
...
CREATE TRIGGER update_daily_subscription_config_updated_at 
    BEFORE UPDATE ON public.daily_subscription_config
    FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
