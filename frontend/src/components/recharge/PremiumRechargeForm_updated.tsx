...
export const PremiumRechargeForm: React.FC<PremiumRechargeFormProps> = ({ 
  ...
  const loadDataPlans = async () => {
    ...
    try {
      const response = await fetch(`/api/v1/networks/${formData.networkProvider}/bundles`);
      const result = await response.json();
