...
export const useOTP = () => {
  ...
  const verifyOTP = useCallback(async (otpCode: string): Promise<boolean> => {
    ...
    try {
      // Call real backend API
      const response = await authApi.verifyOTP(state.phone, otpCode);
